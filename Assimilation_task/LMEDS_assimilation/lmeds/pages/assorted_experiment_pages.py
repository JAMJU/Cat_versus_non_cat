...
class SurveyPage(abstract_pages.AbstractPage):
    # change from nonvalidatingpaage into Abstract page in order to force answer in questionnaire
    ...
    def _getHTMLTxt(self):
        ...
        for item in self.surveyItemList:
            ...
            if elementHTML.strip() == "":
                ...
                # modif to have no numerotaion when depth is bigger than 1
                if item.depth < 1:
                    itemHTML = "%s) %s<br />%s"
                    itemHTML %= (item.enumStrId, item.text, elementHTML)
                elif item.depth >= 1:
                    itemHTML = "%s %s"
                    itemHTML %= (item.text, elementHTML)
...
#         tmpList = outputList
    ...
    def getValidation(self):
        """
        Hard written for survey for the particular case of French/English 2019, we know that only the radio answers must be filled, and only two responses
        :return:
        """
        result = "radio_id = \"\";\n" \
                 + "unfilled = false;\n" \
...
class MediaChoicePage_old(abstract_pages.AbstractPage):
    
    pageName = "media_choice_old"
    ...
    def __init__(self, instructionText, audioOrVideo, pauseDuration,
                 ...
                 timeout=None,
                 *args, **kargs):
        super(MediaChoicePage_old, self).__init__(*args, **kargs)
    ...
    def getOutput(self, form):
        ...
        try:
            value = super(MediaChoicePage_old, self).getOutput(form)
...
# changed for the assimilation task to have examples and answer buttons aligned
class MediaChoicePage(abstract_pages.AbstractPage):
    ...
    def _getHTMLTxt(self):
        ...
        Couple_label_row2 = ""
        Couple_label_row3 = ""
        nb_buttons = len(self.responseButtonList)
        if nb_buttons > 5:
            # we separate the list in three
            nb_1 = nb_buttons // 3
            nb_2 = nb_buttons // 3 + nb_buttons%3 // 2
            nb_3 =  nb_buttons // 3 + nb_buttons%3 // 2 +  (nb_buttons%3)%2
            text_to_add = ""
            for i in range(nb_1):
                radioButtonTxt = radioButton % {'id': i, "disabled": disabledTxt}
                text = self.textDict[self.responseButtonList[i]]
                Couple_label_row1 += "<td class='responses'>%s</td>" % text
                text_to_add += "<td class='responses'>%s</td>" % radioButtonTxt

            Couple_label_row1 += "</tr>\n<tr>"
            Couple_label_row1 += text_to_add

            text_to_add = ""

            for i in range(nb_2):
                radioButtonTxt = radioButton % {'id': i + nb_1, "disabled": disabledTxt}
                text = self.textDict[self.responseButtonList[i + nb_1]]
                Couple_label_row2 += "<td class='responses'>%s</td>" % text
                text_to_add += "<td class='responses'>%s</td>" % radioButtonTxt

            Couple_label_row2 += "</tr>\n<tr>"
            Couple_label_row2 += text_to_add

            text_to_add = ""

            for i in range(nb_3):
                radioButtonTxt = radioButton % {'id': i + nb_1 + nb_2, "disabled": disabledTxt}
                text = self.textDict[self.responseButtonList[i + nb_1 + nb_2]]
                Couple_label_row3 += "<td class='responses'>%s</td>" % text
                text_to_add += "<td class='responses'>%s</td>" % radioButtonTxt

            Couple_label_row3 += "</tr>\n<tr>"
            Couple_label_row3 += text_to_add
            return htmlTxt % (Couple_label_row1, Couple_label_row2, Couple_label_row3)
        else:
            text_to_add = ""
            ...
            Couple_label_row1 += "</tr>\n<tr>"
            Couple_label_row1 += text_to_add
            return htmlTxt % (Couple_label_row1, '', '')
    ...
    def getHTML(self):
        ...
        if len(self.mediaList) < 4:
            ...
            first = len(self.mediaList) // 3
            second = len(self.mediaList) // 3
            last = len(self.mediaList) - first - second
            ...
            playBtnLabelRow2 = ''
            playBtnSnippet2 = ''
            template2 = "<td class='buttons'>%s</td>"
            for i in range(second):
                audioLabel = self.textDict['play_button']
                mediaButtonHTML = audio.generateAudioButton(self.mediaList[i + first], i + first,
                                                            audioLabel,
                                                            self.pauseDuration,
                                                            False)
                if self.buttonLabelList is not None:
                    label = self.textDict[self.buttonLabelList[i + first]]
                    playBtnLabelRow2 += template2 % label
                playBtnSnippet2 += template2 % mediaButtonHTML

            # Add optional button labels
            playBtnSnippet2 = '<tr>%s</tr>' % playBtnSnippet2
            if self.buttonLabelList is not None:
                playBtnLabelRow2 = '<tr>%s</tr>' % playBtnLabelRow2

            playBtnSnippet += playBtnLabelRow2 + playBtnSnippet2

            playBtnLabelRow3 = ''
            playBtnSnippet3 = ''
            template3 = "<td class='buttons'>%s</td>"
            for i in range(last):
                audioLabel = self.textDict['play_button']
                mediaButtonHTML = audio.generateAudioButton(self.mediaList[i + first + second], i + first + second,
                                                            audioLabel,
                                                            self.pauseDuration,
                                                            False)
                if self.buttonLabelList is not None:
                    label = self.textDict[self.buttonLabelList[i + first + second]]
                    playBtnLabelRow3 += template3 % label
                playBtnSnippet3 += template3 % mediaButtonHTML

            # Add optional button labels
            playBtnSnippet3 = '<tr>%s</tr>' % playBtnSnippet3
            if self.buttonLabelList is not None:
                playBtnLabelRow3 = '<tr>%s</tr>' % playBtnLabelRow3

            playBtnSnippet += playBtnLabelRow3 + playBtnSnippet3
...
# changed to be able to have two rows of button for the french/english first experiment, the old version is above
class MediaChoicePage_changed(abstract_pages.AbstractPage):
    ...
    def __init__(self, instructionText, audioOrVideo, pauseDuration,
                 ...
                 timeout=None,
                 *args, **kargs):
        super(MediaChoicePage_changed, self).__init__(*args, **kargs)
    ...
    def _getHTMLTxt(self):
        ...
        Couple_label_row2 = ""
        nb_buttons = len(self.responseButtonList)
        # we separate the list in two
        nb_1 = int(nb_buttons / 2)
        nb_2 =int( nb_buttons / 2) + nb_buttons%2
        text_to_add = ""
        for i in range(nb_1):
            radioButtonTxt = radioButton % {'id': i, "disabled": disabledTxt}
            text = self.textDict[self.responseButtonList[i]]
        ...
        return htmlTxt % (Couple_label_row1, Couple_label_row2)
    ...
    def getOutput(self, form):
        ...
        try:
            value = super(MediaChoicePage_changed, self).getOutput(form)
            if not self._doPlayMedia():
                value += ",0"
