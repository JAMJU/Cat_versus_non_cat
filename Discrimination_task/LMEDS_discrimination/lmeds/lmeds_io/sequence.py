...
def get_answer(line):
    ...
    newline = line.replace('\n', '').split(';')[-1].split(',')
    answer_dict = {1:'1_sure', 2:'1_almost_sure', 3:'1_unsure', 4:'2_sure', 5:'2_almost_sure', 6:'2_unsure'}
    convert = {'1_sure':3, '1_almost_sure':2, '1_unsure':1, '2_sure':-3, '2_almost_sure':-2, '2_unsure':-1}
    for i in range(1,7):
        if newline[i] == '1':
            answer = answer_dict[i]
            return convert[answer]

def get_gold_answer_test(name_file_tested, file_with_answers):
    f = open(file_with_answers, 'r')
    ind = f.readline().replace('\n', '').split('\t')
    for line in f:
        newline = line.replace('\n', '').split('\t')
        if name_file_tested == newline[ind.index('filename')]:
            return 1 if newline[ind.index('TGT_first')] == 'True' else -1
    print('error', name_file_tested, 'not in', file_with_answers)
    return 0
...
def test_ok(filename):
    ...
    for line in f:
        ...
        if previous == 'media_choice' and current == 'media_choice': # we avoid the beginning test
            ...
            if 'hibou' in file_tested or 'dog' in file_tested or 'easy' in file_tested: # test file names
                answer_indiv = get_answer(line)
                answer_test_easy = os.path.join('/'.join(filename.split('/')[:-3]), 'answer_test', 'corres_easy.csv')
                gold_answer = get_gold_answer_test(file_tested, answer_test_easy)
                answer_test.append(1 if answer_indiv*gold_answer > 0 else 0)
                count += 1
        previous = current
...
def is_validated(file_output):
    ...
    try:
        ...
        if not is_ended(file_output):
            return False
        # to uncomment when real test (not pilot)
